...
async def patch_label_session(session_id: str, body: dict):
    """Assigns an action label tag to a session via PATCH method (Zustand store format)"""
    session = db.get_session(session_id)
    if not session:
        raise HTTPException(404, "Session not found")
    label = body.get("label") or body.get("action_label") or "unlabeled"
    notes = body.get("notes", "")
    db.label_session(session_id, label, notes)
    return {"session_id": session_id, "action_label": label}
